                            ...
                            FAR struct sockaddr *addr, socklen_t addrlen)
{
      ...
      if (false == retry)
        {
          ...
          udp->sec = htonl((uint32_t)tv.tv_sec);
          udp->usec = htonl((uint32_t)tv.tv_usec);
          delay = 1;
        }
  ...
  /* iperf2 terminates a UDP test with a negative packet id.  Without this
   * datagram the reference server waits for its socket timeout and prints
   * "recvmsg failed: Connection timed out" even though every data packet
   * arrived.  Send it more than once because UDP completion is intentionally
   * best effort; the server stops on the first copy.
   */
  ...
  for (delay = 0; delay < 3; delay++)
    {
      sendto(sockfd, buffer, want_send, 0, addr, addrlen);
      usleep(1000);
    }
