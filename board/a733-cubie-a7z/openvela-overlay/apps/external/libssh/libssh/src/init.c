...
#if defined(HAVE_CONSTRUCTOR_ATTRIBUTE) && !defined(__NuttX__)
#define CONSTRUCTOR_ATTRIBUTE __attribute__((constructor))
#else
#define CONSTRUCTOR_ATTRIBUTE
#endif /* HAVE_CONSTRUCTOR_ATTRIBUTE */

#if defined(HAVE_DESTRUCTOR_ATTRIBUTE) && !defined(__NuttX__)
...
#ifdef __NuttX__
    /* NuttX built-in applications are statically linked into one image.
     * ELF-style constructors can run before board entropy and the heap are
     * ready, and the upstream reference counter then caches that early
     * failure forever.  NuttX therefore initializes explicitly from the
     * first ssh/scp/sshd command.  If a transient initialization still
     * fails, do not retain a poisoned reference: unwind completed stages and
     * allow the next command to retry.  Report the precise failing stage so
     * a board log is actionable rather than just saying "ssh_init failed".
     */
    if (rc != SSH_OK) {
        fprintf(stderr, "libssh: initialization stage %d failed (%d)\n",
                stage, rc);
        if (stage > 3) {
            ssh_socket_cleanup();
        }
...
#ifdef __NuttX__
    /* Built-in NuttX applications share one address space.  Destroying the
     * process-global crypto/thread state when one command exits makes a later
     * ssh/scp/sshd invocation fail to initialize.  Keep the constructor-level
     * reference alive for the lifetime of the firmware while still balancing
     * explicit per-command references.
     */
    if (!destructor) {
        ssh_mutex_lock(&ssh_init_mutex);
        if (_ssh_initialized > 1) {
