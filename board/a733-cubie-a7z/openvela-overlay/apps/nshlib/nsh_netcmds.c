...
typedef enum
{
  ...
  ARP_DISABLE                /* Set the NOARP flag for ther interface to disable the arp learning function */
} arp_cfg_e;
#endif
...
int cmd_wget(FAR struct nsh_vtbl_s *vtbl, int argc, FAR char **argv)
{
  ...
  /* This NSH webclient has no TLS operations attached.  Fail before touching
   * the output; curl provides certificate-verified HTTPS on this board.
   */
  if (strncmp(url, "http://", 7) != 0)
    {
      nsh_error(vtbl, "wget: HTTP only; use curl for HTTPS\n");
      return ERROR;
    }
  ...
  if (ret >= 0 && (ctx.http_status < 200 || ctx.http_status >= 300))
    {
      nsh_error(vtbl, "wget: HTTP status %u\n", ctx.http_status);
      ret = -EIO;
    }
  if (ret < 0)
    {
  ...
  if (fsync(fd) < 0)
    {
      nsh_error(vtbl, g_fmtcmdfailed, argv[0], "fsync", NSH_ERRNO);
      ret = ERROR;
    }
  ...
  if (fd >= 0)
    {
      if (close(fd) < 0 && ret >= 0)
        {
          nsh_error(vtbl, g_fmtcmdfailed, argv[0], "close", NSH_ERRNO);
          ret = ERROR;
        }
    }
